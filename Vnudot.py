...
plt.plot(xtraining1, ytraining1, 'r*')
plt.savefig('residualsmodel.png')
Ulim = ypredict + 2*np.sqrt(yvariance)
Llim = ypredict - 2*np.sqrt(yvariance)
print model

par = np.zeros(2)
par[0] = np.sqrt(model['rbf_variance']) # use the optimized amplitude
par[1] = model['rbf_lengthscale'] # use the optimized lengthscale
print par[0], par[1]
K1 = kernel.K(xtraining1, xtraining1)
#K1 = Vf.SE(xtraining1, xtraining1,par[0],par[1])
print "diag",np.diag(K1)
print "det:",np.linalg.det(np.matrix(K1))
...
if (args.diagnosticplots):
    ...
    plt.ylim(np.median(Llim2[:,0])-np.std(Llim2[:,0]),np.median(Llim2[:,0])+np.std(Llim2[:,0]))
    #x1,x2,y1,y2 = plt.axis()
    #plt.axis((x1,x2,np.min(Llim[10:-10]), np.max(Ulim[10:-10])))
