...
for i in range(len(outfiles)):
   ...
   # print "READBINS IS",i,datfiles[i]
...
# yaxis = np.linspace(leftbin/allbins, rightbin/allbins, bins)
    ...
    if i == 0 or i == 1:
        Vf.makemap(inferredarray, -limitdifference, limitdifference, mjdinfer, yaxis[i], mjd, 'MJD', 'Fraction Of Pulse Period', pulsar, '{0}_inferreddata.png'.format(outfiles[i]), peakline=allbins/4-leftbin)
        Vf.makemap(linferredarray, -np.log10(limitdifference), np.log10(limitdifference), mjdinfer, yaxis[i], mjd, 'MJD', 'Fraction Of Pulse Period', pulsar, '{0}_linferreddata.png'.format(outfiles[i]), peakline=allbins/4-leftbin)
        Vf.makemap(inferredvar, 0 , np.amax(inferredvar), mjdinfer, yaxis[i], mjd, 'MJD', 'Fraction Of Pulse Period', pulsar, '{0}_inferredvariance.png'.format(outfiles[i]), peakline=allbins/4-leftbin)

    else:
        Vf.makemap(inferredarray, -limitdifference, limitdifference, mjdinfer, yaxis[i], mjd, 'MJD', 'Fraction Of Pulse Period', pulsar, '{0}_inferreddata.png'.format(outfiles[i]))
        Vf.makemap(linferredarray, -np.log10(limitdifference), np.log10(limitdifference), mjdinfer, yaxis[i], mjd, 'MJD', 'Fraction Of Pulse Period', pulsar, '{0}_linferreddata.png'.format(outfiles[i]))
        Vf.makemap(inferredvar, 0 , np.amax(inferredvar), mjdinfer, yaxis[i], mjd, 'MJD', 'Fraction Of Pulse Period', pulsar, '{0}_inferredvariance.png'.format(outfiles[i]))
